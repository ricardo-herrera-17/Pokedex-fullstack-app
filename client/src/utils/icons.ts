...
export function searchIcon(pokemonType: string) {
  const pokemonTypePNG = Object.entries(icons).filter(
    (icon) => icon[0] === pokemonType
  );

  if (pokemonTypePNG) return pokemonTypePNG[0][1];
  return null;
}
